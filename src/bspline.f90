...
contains
    ...
    real(8),dimension(:),intent(in)        :: fcn    !! `(nx)` array of function values to interpolate. `fcn(i)` should
    ...
    real(8),dimension(:),intent(inout)     :: tx     !! The `(nx+kx)` knots in the \(x\) direction
    ...
    real(8),dimension(:),intent(out)       :: bcoef  !! `(nx)` array of coefficients of the b-spline interpolant.
    ...
    real(8),dimension(2*kx*(nx+1)) :: work
    logical :: status_ok
...
!  * Jacob Williams, 10/30/2015 : Created 1D routine.
    ...
    real(8),dimension(nx),intent(in)    :: bcoef    !! the b-spline coefficients computed by [[db1ink]].
...
!  * Jacob Williams, 2/24/2015 : extensive refactoring of CMLIB routine.
    ...
    real(8),dimension(:),intent(in)         :: x     !! `(nx)` array of \(x\) abcissae. must be strictly increasing.
    real(8),dimension(:),intent(in)         :: y     !! `(ny)` array of \(y\) abcissae. must be strictly increasing.
    real(8),dimension(:),intent(in)         :: z     !! `(nz)` array of \(z\) abcissae. must be strictly increasing.
    real(8),dimension(:,:,:),intent(in)     :: fcn   !! `(nx,ny,nz)` matrix of function values to interpolate. `fcn(i,j,k)` should
    ...
    real(8),dimension(:),intent(inout)      :: ty    !! The `(ny+ky)` knots in the \(y\) direction for the spline interpolant.
                                                      ...
                                                      !!
                                                      !! Must be non-decreasing.
    real(8),dimension(:),intent(inout)      :: tz    !! The `(nz+kz)` knots in the \(z\) direction for the spline interpolant.
                                                      !!
                                                      !! * If `iknot=0` these are chosen by [[db3ink]].
                                                      !! * If `iknot=1` these are specified by the user.
                                                      !!
                                                      !! Must be non-decreasing.
    real(8),dimension(:,:,:),intent(out)    :: bcoef !! `(nx,ny,nz)` matrix of coefficients of the b-spline interpolant.
    integer,intent(out)                      :: iflag !! *  0 = successful execution.
                                                      !! *  2 = `iknot` out of range.
                                                      !! *  3 = `nx` out of range.
                                                      !! *  4 = `kx` out of range.
    ...
    real(8),dimension(max(2*kx*(nx+1),2*ky*(ny+1),2*kz*(nz+1))) :: work
    logical :: status_ok
...
!  * Jacob Williams, 2/24/2015 : extensive refactoring of CMLIB routine.
    ...
    real(8),intent(in)                     :: xval     !! \(x\) coordinate of evaluation point.
    real(8),intent(in)                     :: yval     !! \(y\) coordinate of evaluation point.
    real(8),intent(in)                     :: zval     !! \(z\) coordinate of evaluation point.
    real(8),dimension(nx+kx),intent(in)    :: tx       !! sequence of knots defining the piecewise polynomial
                                                        !! in the \(x\) direction. (same as in last call to [[db3ink]])
    real(8),dimension(ny+ky),intent(in)    :: ty       !! sequence of knots defining the piecewise polynomial
                                                        !! in the \(y\) direction. (same as in last call to [[db3ink]])
    real(8),dimension(nz+kz),intent(in)    :: tz       !! sequence of knots defining the piecewise polynomial
                                                        !! in the \(z\) direction. (same as in last call to [[db3ink]])
    real(8),dimension(nx,ny,nz),intent(in) :: bcoef    !! the b-spline coefficients computed by [[db3ink]].
    real(8),intent(out)                    :: f        !! interpolated value
    integer,intent(out)                     :: iflag    !! status flag:
                                                        !!
                                                        !! * \( = 0 \)   : no errors
    ...
    real(8),dimension(ky,kz)           :: temp1
    real(8),dimension(kz)              :: temp2
    real(8),dimension(3*max(kx,ky,kz)) :: work
...
!  * Jacob Williams, 2/24/2015 : Created this routine.
    ...
    real(8),dimension(:),intent(in)            :: y     !! `(ny)` array of \(y\) abcissae. must be strictly increasing.
    real(8),dimension(:),intent(in)            :: z     !! `(nz)` array of \(z\) abcissae. must be strictly increasing.
    real(8),dimension(:),intent(in)            :: q     !! `(nq)` array of \(q\) abcissae. must be strictly increasing.
    real(8),dimension(:,:,:,:),intent(in)      :: fcn   !! `(nx,ny,nz,nq)` matrix of function values to interpolate. `fcn(i,j,k,q)` should
    ...
    real(8),dimension(:),intent(inout)         :: tz    !! The `(nz+kz)` knots in the z direction for the spline interpolant.
                                                         ...
                                                         !!
                                                         !! Must be non-decreasing.
    real(8),dimension(:),intent(inout)         :: tq    !! The `(nq+kq)` knots in the q direction for the spline interpolant.
                                                         !!
                                                         !! * If `iknot=0` these are chosen by [[db4ink]].
                                                         !! * If `iknot=1` these are specified by the user.
                                                         !!
                                                         !! Must be non-decreasing.
    real(8),dimension(:,:,:,:),intent(out)     :: bcoef !! `(nx,ny,nz,nq)` matrix of coefficients of the b-spline interpolant.
    integer,intent(out)                         :: iflag !! *  0 = successful execution.
                                                         !! *  2 = `iknot` out of range.
                                                         !! *  3 = `nx` out of range.
                                                         !! *  4 = `kx` out of range.
    ...
    real(8),dimension(nx*ny*nz*nq) :: temp
    real(8),dimension(max(2*kx*(nx+1),2*ky*(ny+1),2*kz*(nz+1),2*kq*(nq+1))) :: work
    logical :: status_ok
...
!  * Jacob Williams, 2/24/2015 : Created this routine.
    ...
    real(8),intent(in)                        :: yval     !! \(y\) coordinate of evaluation point.
    real(8),intent(in)                        :: zval     !! \(z\) coordinate of evaluation point.
    real(8),intent(in)                        :: qval     !! \(q\) coordinate of evaluation point.
    real(8),dimension(nx+kx),intent(in)       :: tx       !! sequence of knots defining the piecewise polynomial
    ...
    real(8),dimension(nq+kq),intent(in)       :: tq       !! sequence of knots defining the piecewise polynomial
                                                           !! in the \(q\) direction. (same as in last call to [[db4ink]])
    real(8),dimension(nx,ny,nz,nq),intent(in) :: bcoef    !! the b-spline coefficients computed by [[db4ink]].
    real(8),intent(out)                       :: f        !! interpolated value
    integer,intent(out)                        :: iflag    !! status flag:
                                                           !!
                                                           !! * \( = 0 \)   : no errors
    ...
    real(8),dimension(ky,kz,kq)             :: temp1
    real(8),dimension(kz,kq)                :: temp2
    real(8),dimension(kq)                   :: temp3
    real(8),dimension(3*max(kx,ky,kz,kq))   :: work
...
!  * Jacob Williams, 2/24/2015 : Created this routine.
    ...
    real(8),dimension(:),intent(in)               :: z     !! `(nz)` array of \(z\) abcissae. must be strictly increasing.
    real(8),dimension(:),intent(in)               :: q     !! `(nq)` array of \(q\) abcissae. must be strictly increasing.
    real(8),dimension(:),intent(in)               :: r     !! `(nr)` array of \(r\) abcissae. must be strictly increasing.
    real(8),dimension(:,:,:,:,:),intent(in)       :: fcn   !! `(nx,ny,nz,nq,nr)` matrix of function values to interpolate. `fcn(i,j,k,q,r)` should
    ...
    real(8),dimension(:),intent(inout)            :: tq    !! The `(nq+kq)` knots in the \(q\) direction for the spline interpolant.
                                                            ...
                                                            !!
                                                            !! Must be non-decreasing.
    real(8),dimension(:),intent(inout)            :: tr    !! The `(nr+kr)` knots in the \(r\) direction for the spline interpolant.
                                                            !!
                                                            !! * If `iknot=0` these are chosen by [[db5ink]].
                                                            !! * If `iknot=1` these are specified by the user.
                                                            !!
                                                            !! Must be non-decreasing.
    real(8),dimension(:,:,:,:,:),intent(out)      :: bcoef !! `(nx,ny,nz,nq,nr)` matrix of coefficients of the b-spline interpolant.
    integer,intent(out)                            :: iflag !! *  0 = successful execution.
                                                            !! *  2 = `iknot` out of range.
                                                            !! *  3 = `nx` out of range.
                                                            !! *  4 = `kx` out of range.
    ...
    real(8),dimension(nx*ny*nz*nq*nr) :: temp
    real(8),dimension(max( 2*kx*(nx+1),&
                            2*ky*(ny+1),&
                            2*kz*(nz+1),&
                            2*kq*(nq+1),&
                            2*kr*(nr+1) )) :: work
...
!  * Jacob Williams, 2/24/2015 : Created this routine.
    ...
    real(8),intent(in)                           :: zval     !! \(z\) coordinate of evaluation point.
    real(8),intent(in)                           :: qval     !! \(q\) coordinate of evaluation point.
    real(8),intent(in)                           :: rval     !! \(r\) coordinate of evaluation point.
    real(8),dimension(nx+kx),intent(in)          :: tx       !! sequence of knots defining the piecewise polynomial
                                                              !! in the \(x\) direction.
                                                              !! (same as in last call to [[db5ink]])
    real(8),dimension(ny+ky),intent(in)          :: ty       !! sequence of knots defining the piecewise polynomial
    ...
    real(8),dimension(nq+kq),intent(in)          :: tq       !! sequence of knots defining the piecewise polynomial
                                                              !! in the \(q\) direction.
                                                              !! (same as in last call to [[db5ink]])
    real(8),dimension(nr+kr),intent(in)          :: tr       !! sequence of knots defining the piecewise polynomial
                                                              !! in the \(r\) direction.
                                                              !! (same as in last call to [[db5ink]])
    real(8),dimension(nx,ny,nz,nq,nr),intent(in) :: bcoef    !! the b-spline coefficients computed by [[db5ink]].
    real(8),intent(out)                          :: f        !! interpolated value
    integer,intent(out)                           :: iflag    !! status flag:
                                                              !!
                                                              !! * \( = 0 \)   : no errors
    ...
    real(8),dimension(ky,kz,kq,kr)           :: temp1
    real(8),dimension(kz,kq,kr)              :: temp2
    real(8),dimension(kq,kr)                 :: temp3
    real(8),dimension(kr)                    :: temp4
    real(8),dimension(3*max(kx,ky,kz,kq,kr)) :: work
...
!  * Jacob Williams, 2/24/2015 : Created this routine.
    ...
    integer,intent(in)                                :: ks    !! the order of spline pieces in \(s\)
                                                               !! ( \( 2 \le k_s < n_s \) )
                                                               !! (order = polynomial degree + 1)
    real(8),dimension(:),intent(in)                  :: x     !! `(nx)` array of \(x\) abcissae.
                                                               !! must be strictly increasing.
    real(8),dimension(:),intent(in)                  :: y     !! `(ny)` array of \(y\) abcissae.
                                                               !! must be strictly increasing.
    real(8),dimension(:),intent(in)                  :: z     !! `(nz)` array of \(z\) abcissae.
                                                               !! must be strictly increasing.
    real(8),dimension(:),intent(in)                  :: q     !! `(nq)` array of \(q\) abcissae.
                                                               !! must be strictly increasing.
    real(8),dimension(:),intent(in)                  :: r     !! `(nr)` array of \(r\) abcissae.
                                                               !! must be strictly increasing.
    real(8),dimension(:),intent(in)                  :: s     !! `(ns)` array of \(s\) abcissae.
                                                               !! must be strictly increasing.
    real(8),dimension(:,:,:,:,:,:),intent(in)        :: fcn   !! `(nx,ny,nz,nq,nr,ns)` matrix of function values to interpolate.
    ...
    real(8),dimension(:),intent(inout)               :: tr    !! The `(nr+kr)` knots in the \(r\) direction for the spline interpolant.
                                                               ...
                                                               !!
                                                               !! Must be non-decreasing.
    real(8),dimension(:),intent(inout)               :: ts    !! The `(ns+ks)` knots in the \(s\) direction for the spline interpolant.
                                                               !!
                                                               !! * If `iknot=0` these are chosen by [[db6ink]].
                                                               !! * If `iknot=1` these are specified by the user.
                                                               !!
                                                               !! Must be non-decreasing.
    real(8),dimension(:,:,:,:,:,:),intent(out)       :: bcoef !! `(nx,ny,nz,nq,nr,ns)` matrix of coefficients of the b-spline interpolant.
    integer,intent(out)                               :: iflag !! *  0 = successful execution.
                                                               !! *  2 = `iknot` out of range.
                                                               !! *  3 = `nx` out of range.
                                                               !! *  4 = `kx` out of range.
    ...
    real(8),dimension(nx*ny*nz*nq*nr*ns) :: temp
    real(8),dimension(max( 2*kx*(nx+1),&
                            2*ky*(ny+1),&
                            2*kz*(nz+1),&
                            2*kq*(nq+1),&
...
!  * Jacob Williams, 2/24/2015 : Created this routine.
    ...
    real(8),intent(in)                              :: qval     !! \(q\) coordinate of evaluation point.
    real(8),intent(in)                              :: rval     !! \(r\) coordinate of evaluation point.
    real(8),intent(in)                              :: sval     !! \(s\) coordinate of evaluation point.
    real(8),dimension(nx+kx),intent(in)             :: tx       !! sequence of knots defining the piecewise polynomial
                                                                 !! in the \(x\) direction.
                                                                 !! (same as in last call to [[db6ink]])
    real(8),dimension(ny+ky),intent(in)             :: ty       !! sequence of knots defining the piecewise polynomial
    ...
    real(8),dimension(nr+kr),intent(in)             :: tr       !! sequence of knots defining the piecewise polynomial
                                                                 !! in the \(r\) direction.
                                                                 !! (same as in last call to [[db6ink]])
    real(8),dimension(ns+ks),intent(in)             :: ts       !! sequence of knots defining the piecewise polynomial
                                                                 !! in the \(s\) direction.
                                                                 !! (same as in last call to [[db6ink]])
    real(8),dimension(nx,ny,nz,nq,nr,ns),intent(in) :: bcoef    !! the b-spline coefficients computed by [[db6ink]].
    real(8),intent(out)                             :: f        !! interpolated value
    integer,intent(out)                              :: iflag    !! status flag:
                                                                 !!
                                                                 !! * \( = 0 \)   : no errors
    ...
    real(8),dimension(ky,kz,kq,kr,ks)            :: temp1
    real(8),dimension(kz,kq,kr,ks)               :: temp2
    real(8),dimension(kq,kr,ks)                  :: temp3
    real(8),dimension(kr,ks)                     :: temp4
    real(8),dimension(ks)                        :: temp5
    real(8),dimension(3*max(kx,ky,kz,kq,kr,ks))  :: work
...
!  * Jacob Williams, 5/10/2015 : converted to free-form Fortran.
    ...
    elseif (nrowm1 == 0) then
        if (w(middle,nrow)==0.0_8) iflag = 2
        return
    end if
...
!  Returns a message string associated with the status code.
    ...
    character(len=256),allocatable :: msg    !! status message associated with the flag
