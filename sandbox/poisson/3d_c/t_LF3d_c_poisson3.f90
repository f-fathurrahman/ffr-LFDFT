...
PROGRAM t_poisson
  ...
  DO ip = 1, LF3d%N
    ...
    rho(ip) = exp(-r**2/(2.d0*sigma**2))
    phi_ana(ip) = (2.d0*pi*sigma**2)**(3.d0/2.d0) * erf( r / (sqrt(2.d0)*sigma) ) / r
  ENDDO
  ...
  DO ix = 1, Nx
    ...
    WRITE(333,'(1x,5F18.10)') LF3d%lingrid(1,ip), rho(ip), phi(ip), phi_ana(ip), phi(ip)/phi_ana(ip)
  ENDDO
