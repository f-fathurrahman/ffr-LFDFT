...
SUBROUTINE rdiaghg( n, m, h, s, ldh, e, v )
  ...
  !
  IF ( info > n ) THEN
    WRITE(*,*) 'rdiaghg: S matrix not positive definite', ABS( info )
    STOP
  ENDIF
  !
  IF ( info > 0 ) THEN
    WRITE(*,*) 'rdiaghg: eigenvectors failed to converge', ABS( info )
    STOP
  ENDIF
  !
  IF ( info < 0 ) THEN
    WRITE(*,*) 'rdiaghg: incorrect call to DSYGV*', ABS( info )
    STOP
  ENDIF
