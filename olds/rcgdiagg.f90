...
!----------------------------------------------------------------------------
SUBROUTINE rcgdiagg( npwx, npw, nbnd, psi, e, btype, &
  ...
  IMPLICIT NONE
  !
  INTEGER, PARAMETER :: gstart = 2  !FIXME HARDCODED
  ...
  REAL(DP), INTENT(INOUT) :: psi(npwx,nbnd)
  ...
  !
  npw2 = npw  ! real, not complex
  npwx2 = npwx  ! real, not complex
  !
  ALLOCATE( spsi( npwx ) )
  ALLOCATE( scg(  npwx ) )
  ALLOCATE( hpsi( npwx ) )
  ...
  precondition(:) = 1.0_DP
  WRITE(*,*) 'sum(precondition) = ', sum(precondition)
  !
  avg_iter = 0.D0
  notconv  = 0
  moved    = 0
  ...
  DO m = 1, nbnd
     ...
     !
     CALL h_psi( npwx, npw, 1, psi(1,m), hpsi )
     spsi(:) = psi(:,m)
     !
