...
SUBROUTINE apply_Ham(v, Hv)
  USE m_globals, ONLY : N, Vpsloc, Vhartree
  IMPLICIT NONE
  !
  REAL(8) :: v(N**3)
  REAL(8) :: Hv(N**3)
  ...
  !
  Hv(:) = -0.5d0*Hv(:) + (Vpsloc(:) + Vhartree(:))*v(:)
END SUBROUTINE


! Multicolumn version of apply_Ham
! This subroutine is used in Davidson diagonalization routine
