SUBROUTINE interp_Rhoe_a( Rhoe, Rhoe_a)
  ...
  CALL db3ink( x, Nx+1, y, Ny+1, z, Nz+1, interp_Rhoe, kx,ky,kz, iknot, tx,ty,tz, interp_Rhoe, iflag )
!  WRITE(*,*) 'iflag = ', iflag
  ...
  DO ip_a = 1, Npoints_a
    ...
    dy = grid_a(2,ip_a)
    dz = grid_a(3,ip_a)
    CALL db3val( dx, dy, dz, idx,idy,idz, tx,ty,tz, Nx+1,Ny+1,Nz+1,kx,ky,kz, interp_Rhoe,&
                 val, iflag, inbvx, inbvy, inbvz, iloy, iloz )
    Rhoe_a(ip_a) = val
  ENDDO 

  WRITE(*,*) 'sum(Rhoe_a) = ', sum(Rhoe_a)
